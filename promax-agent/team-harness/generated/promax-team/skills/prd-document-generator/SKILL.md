...
If the host runtime exposes the approved telemetry skill and a valid session context, record use after the artifact is produced. Missing telemetry capability must not block the product output, trigger identity interrogation, or cause fabricated logging claims.
