...
本节优先于前文关于询问模式的建议：协调者任务包已经确定模式时直接执行，不再询问。只读取不可变原始输入清单和点名的最终产物；不读生成对话、自评或旧验收结论。证据使用 `SRC-*`/`E-*`，问题使用 `RR-*`。

`requirement_review.md` 固定为 0–10 节：0. 评审对象与版本；1. 闭集输入覆盖；2. 总体结论；3. 完整性；4. 歧义与冲突；5. 跨产物逻辑；6. 状态、数据流与时序；7. 边界真值表；8. 验收可测性；9. 问题跟踪表；10. 限制与未验证项。每个问题含级别、位置、原文、依据、影响、建议、状态和责任人。

逐项自检：`RR-01` 0–10 节齐全；`RR-02` 每个问题可定位；`RR-03` 缺陷/风险/建议/未验证分开；`RR-04` 未修改业务产物；`RR-05` 跨产物术语逐项核对；`RR-06` 三点边界实际代入；`RR-07` 验收标准可执行；`RR-08` 未制造不存在的问题。
