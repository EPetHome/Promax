...
只可视化不可变输入清单及点名分析产物中的可追溯数字，不引入新数据。输出固定记录：0. 输入与版本；1. 图表问题；2. 字段与聚合口径；3. 图表清单；4. 颜色/尺度/排序；5. 交互与空状态；6. 边界真值表；7. 可访问性；8. 浏览器取证；9. 未验证项；10. 追溯矩阵。

逐项自检：`UA-21` 每个图回答明确问题；`UA-22` 轴、单位、分母完整；`UA-23` 不截断尺度误导；`UA-24` 缺失和零值区分；`UA-25` 图表数值可回到 E-*；`UA-26` 未打开浏览器不写通过；`UA-27` 边界显示与分析一致。
