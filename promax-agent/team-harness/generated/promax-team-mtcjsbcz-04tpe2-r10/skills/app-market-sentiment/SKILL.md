...
只分析不可变输入清单中已提供的应用市场评论，不自动采集或联网补齐。来源 `SRC-*`、评论证据 `E-*`。输出固定为：0. 输入、市场与时间窗；1. 样本和清洗；2. 情绪口径；3. 主题与痛点；4. 版本趋势；5. 高频个案；6. 反例与冲突；7. 边界真值表；8. 风险、限制与未验证；9. 追溯矩阵。

逐项自检：`UA-14` 市场/版本/时间窗明确；`UA-15` 评论原文可定位；`UA-16` 声量不外推用户总体；`UA-17` 单条只标个案；`UA-18` 相反评价并列；`UA-19` 未声称持续预警；`UA-20` 边界已求值或诚实不适用。
