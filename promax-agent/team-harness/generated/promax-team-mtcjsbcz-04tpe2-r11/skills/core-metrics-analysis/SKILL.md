...
只分析不可变输入清单中的指标文件。来源 `SRC-*`、数据证据 `E-*`；计算结果写明公式、时间窗、分母、缺失值和输入位置，相关性与因果分开。输出固定为：0. 输入与口径；1. 数据质量；2. 指标基线；3. 趋势；4. 异常；5. 分群/对照；6. 归因假设与替代解释；7. 边界真值表；8. 限制与未验证；9. 追溯矩阵。

逐项自检：`UA-07` 指标定义完整；`UA-08` 分母与时间窗明确；`UA-09` 计算可复算；`UA-10` 缺失值未静默填补；`UA-11` 异常阈值有输入依据；`UA-12` 不把相关写成因果；`UA-13` 边界实际代入。
